...
def play():
    word = get_word(word_list)
    print('Тогда давайте играть в угадайку слов!')
    ...
    while True:        
        letter = input('Введите одну букву или слово целиком: ').upper()
        if is_alpha(letter) == False:
        ...
        if guessed:
            ...
            return
        if tries == 0:
            print(f'Загаданным, оказалось слово - {word}')
            return 
def replay(a):
    if a.lower() == 'да':
        play()
...
print('Вы хотите сыграть в угадайку? (да или нет):')
while True:
        a = input()   
        if a != 'да' and a != 'нет':
